...
SLOTS = 5
# try:
#     WORKLOAD_RATE = int(argv[3])
# except IndexError:
WORKLOAD_RATE = int(argv[1])

# try:
#     NUM_CONSENSUS_NODES = int(argv[2])
# except IndexError:
...
def create_workload_pod(pod_name,num_nodes):
    ...
    retval += write_indent_level(9) + "sleep 60; intkey-workload \\\n"
...
def create_admin_pod(pod_name,num_nodes):
    ...
    # retval += write_indent_level(7) + "- mountPath: /var/lib/grafana/dashboards/config.json \n"
    # retval += write_indent_level(8) + "name: grafana-provision\n"
    # retval += write_indent_level(8) + "readOnly: True\n"
    # retval += write_indent_level(8) + "subPath: config.json\n"
    # retval += write_indent_level(7) + "- mountPath: /var/lib/grafana/dashboards/with_tps.json\n"
    # retval += write_indent_level(8) + "name: grafana-config\n"
    # retval += write_indent_level(8) + "readOnly: True\n"
    # retval += write_indent_level(8) + "subPath: with_tps.json\n"
...
def create_custom_sawtooth_pod(num_pods, pod_name):
    ...
    for i in range(num_pods):
        ...
        retval += "\n"
        # Engine
        retval += write_indent_level(5) + "- name: sawtooth-ddpoa-engine\n"
        retval += write_indent_level(6) + "image: hyperledger/sawtooth-pbft-engine:chime\n"  # TODO: Change me
        
        # retval += write_indent_level(6) + "image: erlingdevold/ddpoa:latest\n"  # TODO: Change me
        retval += write_indent_level(6) + "imagePullPolicy: Always\n"
        retval += write_indent_level(6) + 'workingDir: "/project/consensus/"\n'
        retval += write_indent_level(6) + "command:\n"
        retval += write_indent_level(7) + "- bash\n"
        retval += write_indent_level(6) + "args:\n"
        retval += write_indent_level(7) + "- -c\n"
        # retval += write_indent_level(7) + f'- "sleep {30 if i else 5} && python3 main.py -vvv --connect tcp://$HOSTNAME:5050 --slots {int(1*NUM_CONSENSUS_NODES//3)}" \n'
        retval += write_indent_level(7) + '- "pbft-engine -vv --connect tcp://$HOSTNAME:5050"'

        # retval += write_indent_level(6) + "image: lillepus/sintef-ddpoa-engine:latest\n"  # TODO: Change me
        ...
        retval += "\n"
        # retval += write_indent_level(4) + "initContainers:\n"
        # Big one (VALIDATOR)
        ...
        if i == 0:
            ...
            retval += write_indent_level(
                9) + 'echo sawtooth.consensus.ddpoa.members=["\\"$ddpoa0pub\\",\\"$ddpoa1pub\\",\\"$ddpoa2pub\\",\\"$ddpoa3pub\\",\\"$ddpoa4pub\\""] &&\n'
            ...
            retval += write_indent_level(11) + 'sawtooth.consensus.algorithm.name=pbft\\\n'
            retval += write_indent_level(11) + 'sawtooth.consensus.algorithm.version=1.0 \\\n'
            retval += write_indent_level(11) + 'sawtooth.consensus.pbft.members=["'
            for node in range(num_pods):
                retval += f'\\"$ddpoa{node}pub\\"' + (',' if node < num_pods - 1 else '')
            
            retval += '"] \\\n'
            # retval += write_indent_level(
            #     11) + 'sawtooth.consensus.ddpoa.members=["\\"$ddpoa0pub\\",\\"$ddpoa1pub\\",\\"$ddpoa2pub\\",\\"$ddpoa3pub\\",\\"$ddpoa4pub\\""] \\\n'
            retval += write_indent_level(11) + "sawtooth.publisher.max_batches_per_block=1200 \\\n"
            retval += write_indent_level(11) + "sawtooth.consensus.ddpoa.seats=3 \\\n"
            ...
            retval +=write_indent_level(10) + "--opentsdb-url http://$DDPOA_ADMIN_SERVICE_HOST:8086  \\\n"
            ...
            retval += write_indent_level(9) + "fi && \n"
            retval += write_indent_level(9) + "sleep 10 && \n"
        ...
        retval += write_indent_level(2) + "selector:\n"
        retval += write_indent_level(3) + f"name: ddpoa-{i}\n"
        retval += write_indent_level(2) + "ports:\n"
...
def create_kubernetes_file():
    ...
    with open("kubernetes_test_file.yml", "w") as outfile:
        ...
        #outfile.write("services:\n")
        outfile.write(create_custom_sawtooth_pod(NUM_CONSENSUS_NODES, "ddpoa") + create_admin_pod('ddpoa',NUM_CONSENSUS_NODES) + create_workload_pod('ddpoa',NUM_CONSENSUS_NODES))
