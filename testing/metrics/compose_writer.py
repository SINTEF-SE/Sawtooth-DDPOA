...
parser.add_argument('-r','--rate', default=4,type=int, help='workload rate')
...
def create_admin_pod(pod_name,num_nodes):
    ...
    retval += write_indent_level(4) + "nodePort: 30086\n"
...
def create_custom_sawtooth_pod(num_pods, pod_name):
    ...
    for i in range(num_pods):
        engines = {"ddpoa" : {"command":f'sleep {30 if i else 5} && python3 main.py -vvv --connect tcp://$HOSTNAME:5050',"image":"ddpoa:latest","version":"0.1","name":"ddpoa"},
          "pbft" : {"command":"pbft-engine -vv --connect tcp://$HOSTNAME:5050", "image":"hyperledger/sawtooth-pbft-engine:chime","version":"1.0","name":"pbft"},
          "poet" : {"command":"poet-engine -vv --connect tcp://$HOSTNAME:5050", "image":"hyperledger/sawtooth-poet-engine:chime","version":"0.1","name":"PoET"},
            }
        ...
        if i == 0:
            ...
            retval += write_indent_level(11) + f'sawtooth.consensus.algorithm.name={engines[pod_name]["name"]}\\\n'
